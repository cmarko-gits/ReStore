  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        // Napomena: agent.Account.refreshToken očekuje token objekte
        const userDataString = localStorage.getItem("userData");
        if (!userDataString) throw new Error("No user data in localStorage");

        const userData = JSON.parse(userDataString);

        // Poziv refresh token endpointa
        const newUser = await agent.Account.refreshToken({
          accessToken: userData.token,
          refreshToken: userData.refreshToken,
        });

        // Update localStorage i Redux store
        localStorage.setItem("userData", JSON.stringify(newUser));
        store.dispatch({ type: 'account/setUser', payload: newUser });
...
const Basket = {
  ...
  removeItem: (productId: number, quantity = 1) =>
    requests.delete(`Basket/basket/item?productId=${productId}&quantity=${quantity}`),
};
...
const Account = {
  ...
  refreshToken: (tokens: { accessToken: string; refreshToken: string }) =>
    requests.post('Account/refresh', tokens),
};
